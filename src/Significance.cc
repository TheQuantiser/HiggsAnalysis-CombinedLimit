  ...
  if (minimizerAlgo.find(',') != std::string::npos) {
    ...
    if (verbose > 1) {
      ...
      CombineLogger::instance().log(
          ...
          __LINE__,
          std::string(
              Form("Using minimizer %s with algorithm %s and tolerance %.3g", type.c_str(), algo.c_str(), tolerance)),
          __func__);
    }
    ...
    if (verbose > 1) {
      ...
      CombineLogger::instance().log(
          "Significance.cc",
          __LINE__,
          std::string(Form("Using minimizer %s with tolerance %.3g", minimizerAlgo.c_str(), tolerance)),
          __func__);
    }
    ...
    if (useMinos_ || bruteForce_) {
      ...
      if (std::isnan(limit) || bruteForce_) {
        std::pair<double, double> le = upperLimitBruteForce(
            *mc_s->GetPdf(), data, *r, mc_s->GetNuisanceParameters(), 1e-3 * minimizerTolerance_, cl);
        limit = le.first;
        limitErr = le.second;
      }
    } else {
  ...
  if (verbose >= 0) {
    if (success) {
      ...
      if (limitErr) {
        ...
        std::cout << "Limit: " << r->GetName() << (lowerLimit_ ? " > " : " < ") << limit << " @ " << cl * 100 << "% CL"
                  << std::endl;
      }
    }
  }
...
double Significance::upperLimitWithMinos(
    RooAbsPdf &pdf, RooAbsData &data, RooRealVar &poi, const RooArgSet *nuisances, double tolerance, double cl) const {
  ...
  double limit = 0.0;
  {
    ...
    if (minosStat == -1) {
      minim.hesse();
      CombineLogger::instance().log("Significance.cc", __LINE__, "Minos failed; using Hesse errors", __func__);
      limit = poi.getVal() + (lowerLimit_ ? -poi.getError() : poi.getError());
    } else {
      limit = poi.getVal() + (lowerLimit_ ? poi.getAsymErrorLo() : poi.getAsymErrorHi());
    }
  }
...
std::pair<double, double> Significance::upperLimitBruteForce(
  ...
  double rval = poi.getVal() + (lowerLimit_ ? -3 : +3) * poi.getError(), rlow = poi.getVal(),
         rhigh = lowerLimit_ ? poi.getMin() : poi.getMax();
