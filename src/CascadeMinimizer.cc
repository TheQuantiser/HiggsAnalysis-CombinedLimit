...
bool CascadeMinimizer::freezeDiscParams(const bool freeze) {
  ...
  if (freezeDisassParams) {
    ...
    bool ret =
        utils::freezeAllDisassociatedRooMultiPdfParameters((CascadeMinimizerGlobalConfigs::O().allRooMultiPdfs),
...
bool CascadeMinimizer::improve(int verbose, bool cascade, bool forceResetMinimizer) {
  ...
  if (forceResetMinimizer || !minimizer_.get())
    remakeMinimizer();
    ...
    if (cascade && !outcome && !fallbacks_.empty()) {
      ...
      if (verbose > 0) {
        ...
        CombineLogger::instance().log("CascadeMinimizer.cc",
                                      __LINE__,
                                      std::string(Form("Failed minimization with %s, %s and tolerance %g",
                                                       nominalType.c_str(),
                                                       nominalAlgo.c_str(),
                                                       nominalTol)),
                                      __func__);
      ...
      for (std::vector<Algo>::const_iterator it = fallbacks_.begin(), ed = fallbacks_.end(); it != ed; ++it) {
        Significance::MinimizerSentry minimizerConfig(
            it->type + "," + it->algo,
            it->tolerance != Algo::default_tolerance() ? it->tolerance : nominalTol);  // set the global defaults
        int myStrategy = it->strategy;
        if (myStrategy == Algo::default_strategy())
        ...
        if (nominalType != ROOT::Math::MinimizerOptions::DefaultMinimizerType() ||
          ...
          if (verbose > 0) {
            ...
            CombineLogger::instance().log(
                "CascadeMinimizer.cc",
                __LINE__,
                std::string(Form("Will fall back to minimization using %s, strategy %d and tolerance %g",
                                 (it->algo).c_str(),
                                 myStrategy,
                                 it->tolerance)),
                __func__);
  ...
  if (oldFallback_) {
    ...
    if (verbose + 2 > 0)
      CombineLogger::instance().log(
          "CascadeMinimizer.cc",
          __LINE__,
          std::string(Form("Minimization configured with Type=%s, Algo=%s, strategy=%d, tolerance=%g",
                           myType.c_str(),
                           myAlgo.c_str(),
                           myStrategy,
                           tol)),
    ...
    if (lastHesse_ && !noHesse) {
      ...
      if (verbose + 2 > 0)
        CombineLogger::instance().log(
            "CascadeMinimizer.cc", __LINE__, std::string(Form("Hesse finished with status=%d", status)), __func__);
    }
    if (simnll)
    ...
    if (status == 1) {
      std::cerr << "[WARNING] Minimization finished with status 1 (covariance matrix forced positive definite), this "
                   "could indicate a problem with the minimum!"
                << std::endl;
      if (verbose + 2 > 0)
        CombineLogger::instance().log("CascadeMinimizer.cc",
                                      __LINE__,
                                      "[WARNING] Minimization finished with status 1 (covariance matrix forced "
                                      "positive definite), this could indicate a problem with the minimum.",
                                      __func__);
    }
    if (verbose + 2 > 0) {
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, std::string(Form("Minimization finished with status=%d", status)), __func__);
    }
  }
  if (verbose + 2 > 0) {
    if (outcome)
      CombineLogger::instance().log("CascadeMinimizer.cc", __LINE__, "Minimization success! status=0", __func__);
    else
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "Minimization ended with latest status != 0 or 1", __func__);
  }
...
bool CascadeMinimizer::minos(const RooArgSet &params, int verbose) {
  ...
  freezeDiscParams(true);
  // need to re-run minimization before error evaluation
  minimizer_->minimize(myType.c_str(), myAlgo.c_str());
  int iret = 0;
  {
    std::string minAlgo =
  ...
  if (iret == 0) {
    minimizer_->hesse();
    if (verbose > 0)
      CombineLogger::instance().log("CascadeMinimizer.cc", __LINE__, "Minos failed; using Hesse errors", __func__);
  } else if (verbose > 0) {
    CombineLogger::instance().log(
        "CascadeMinimizer.cc", __LINE__, std::string(Form("Minos finished with status=%d", iret)), __func__);
  }
  freezeDiscParams(false);
...
bool CascadeMinimizer::iterativeMinimize(double &minimumNLL, int verbose, bool cascade) {
  ...
  tw.Stop();
  if (verbose > 2)
    std::cout << "Full fit done in " << tw.RealTime() << std::endl;
...
bool CascadeMinimizer::minimize(int verbose, bool cascade) {
  ...
  if (runtimedef::get(std::string("MINIMIZER_skipDiscreteIterations")))
    doMultipleMini = false;
...
bool CascadeMinimizer::multipleMinimize(const RooArgSet &reallyCleanParameters,
  ...
  static bool maskConstraints =
      freezeDisassParams && runtimedef::get(std::string("MINIMIZER_multiMin_maskConstraints"));
  ...
  if (hideConstants && simnll) {
    ...
    if (maskConstraints)
      simnll->setMaskConstraints(true);
  ...
  if ((mode == 0) /*&& runShortCombinations )*/ || mode == 1)
    myCombos = utils::generateOrthogonalCombinations(pdfSizes);
  else
    myCombos = utils::generateCombinations(pdfSizes);
  ...
  if (mode != 0)
    my_it++;  // already did the best fit case
  ...
  for (; my_it != myCombos.end(); my_it++) {
    ...
    for (std::vector<int>::iterator it = cit.begin(); it != cit.end(); it++) {
    ...
    if (fitCounter > 0)
      params->assignValueOnly(reallyCleanParameters);  // no need to reset from 0'th fit

    if (maskChannels == 2 && simnll) {
      for (int id = 0; id < numIndeces; id++)
        ((RooCategory *)(pdfCategoryIndeces.at(id)))->setConstant(id != changedIndex && changedIndex != -1);
      simnll->setMaskNonDiscreteChannels(true);
    }
    ...
    if (maskChannels == 2 && simnll) {
      for (int id = 0; id < numIndeces; id++)
        ((RooCategory *)(pdfCategoryIndeces.at(id)))->setConstant(false);
    ...
    if (thisNllValue < minimumNLL) {
      ...
      if (verbose > 2) {
        std::cout << " .... Found a better fit: new NLL = " << thisNllValue
                  << " (improvement: " << (thisNllValue - minimumNLL) << std::endl;
      }
      ...
      for (int id = 0; id < numIndeces; id++) {
        if (bestIndeces[id] != ((RooCategory *)(pdfCategoryIndeces.at(id)))->getIndex())
          newDiscreteMinimum = true;
  ...
  tw.Stop();
  if (verbose > 2)
    std::cout << "Done " << myCombos.size() << " combinations in " << tw.RealTime() << " s. New discrete minimum? "
              << newDiscreteMinimum << std::endl;
  ...
  if (hideConstants && simnll) {
    ...
    if (maskConstraints)
      simnll->setMaskConstraints(false);
    minimizer_.reset();  // will be recreated when needed by whoever needs it
  }
...
void CascadeMinimizer::initOptions() {
  options_.add_options()("cminPoiOnlyFit", "First do a fit floating only the parameter of interest")(
      "cminPreScan", "Do a scan before first minimization")(
      "cminPreFit",
      boost::program_options::value<int>(&preFit_)->default_value(preFit_),
      "if set to a value N > 0, a pre-fit with strategy (N-1) will be performed with the constrained nuisance "
      "parameters frozen.")(
      "cminApproxPreFitTolerance",
      boost::program_options::value<double>(&approxPreFitTolerance_)->default_value(approxPreFitTolerance_),
      "If nonzero, first do a pre-fit with this tolerance (or 10 times the final tolerance, whichever is largest)")(
      ...
      "Strategy to use in the pre-fit")("cminSingleNuisFit",
                                        "First perform a minimization of each nuisance parameter individually")(
      "cminFallbackAlgo",
      ...
      "Set the default minimizer type (e.g. Minuit, Minuit2, GSLMultiMin, Ceres)")(
      "cminDefaultMinimizerAlgo",
      boost::program_options::value<std::string>(&defaultMinimizerAlgo_)->default_value(defaultMinimizerAlgo_),
      "Set the default minimizer algorithm (e.g. Migrad, Simplex, TrustRegion)")(
      ...
      "cminCeresMaxIterations",
      boost::program_options::value<int>()->default_value(1000),
      "Maximum number of iterations for Ceres minimizer")(
      "cminCeresLinearSolver",
      boost::program_options::value<std::string>()->default_value("dense_qr"),
      "Linear solver for Ceres (e.g. dense_qr, dense_normal_cholesky, iterative_schur)")(
      "cminCeresAlgo",
      boost::program_options::value<std::string>()->default_value("TrustRegion"),
      "Algorithm for Ceres minimizer (TrustRegion or LineSearch)")(
      "cminCeresVerbose",
      boost::program_options::value<bool>()->default_value(false),
      "Print full Ceres solver report to combine log")(
      "cminCeresMultiStart",
      boost::program_options::value<unsigned int>()->default_value(1),
      "Run Ceres multiple times with randomised starts and keep best fit")(
      "cminCeresJitter",
      boost::program_options::value<double>()->default_value(1.0),
      "Scale of randomised multi-start perturbations (0 disables jitter)")(
      "cminCeresNumThreads",
      boost::program_options::value<int>()->default_value(1),
      "Number of threads used by the Ceres solver")("cminCeresRandomSeed",
                                                    boost::program_options::value<unsigned int>()->default_value(12345),
                                                    "Random seed for Ceres multi-start initialisation")(
      "cminCeresUseNumericGradient",
      boost::program_options::bool_switch()->default_value(false),
      "Force numeric derivatives even if analytic gradients are available")(
      "cminUseCeres",
      boost::program_options::bool_switch()->default_value(false),
      "Use Ceres minimizer (alias for --cminDefaultMinimizerType=Ceres --cminDefaultMinimizerAlgo=TrustRegion)")(
      "cminCeresFunctionTolerance",
      boost::program_options::value<double>()->default_value(defaultMinimizerTolerance_),
      "Function tolerance for Ceres minimizer")(
      "cminCeresGradientTolerance",
      boost::program_options::value<double>()->default_value(defaultMinimizerTolerance_),
      "Gradient tolerance for Ceres minimizer")(
      "cminCeresParameterTolerance",
      boost::program_options::value<double>()->default_value(defaultMinimizerTolerance_),
      "Parameter tolerance for Ceres minimizer")(
      "cminCeresNumericDiffStep",
      boost::program_options::value<double>()->default_value(1e-4),
      "Multiplier applied to each variable's step for Ceres numeric differentiation")(
      "cminCeresDiffMethod",
      boost::program_options::value<std::string>()->default_value("central"),
      "Numeric differentiation method for Ceres (forward or central)")(
      "cminCeresLossFunction",
      boost::program_options::value<std::string>()->default_value("none"),
      "Loss function for Ceres (none, huber, cauchy)")("cminCeresInitialRadius",
                                                       boost::program_options::value<double>()->default_value(1.0),
                                                       "Initial trust region radius for Ceres")(
      "cminCeresLossScale",
      boost::program_options::value<double>()->default_value(1.0),
      "Scale parameter for Huber/Cauchy loss functions")(
      "cminCeresLogFile",
      boost::program_options::value<std::string>()->default_value(""),
      "File to write full Ceres solver summary")("cminCeresProgress",
                                                 boost::program_options::bool_switch()->default_value(false),
                                                 "Print Ceres iteration progress to stdout")(
      "cminCeresMaxTime",
      boost::program_options::value<double>()->default_value(0.0),
      "Maximum wall-clock time for Ceres in seconds (0 disables)")(
      "cminCeresJitterDist",
      boost::program_options::value<std::string>()->default_value("uniform"),
      "Distribution for multi-start jitter (uniform or gaussian)")(
      "cminCeresBoundRelax",
      boost::program_options::value<double>()->default_value(0.0),
...
void CascadeMinimizer::applyOptions(const boost::program_options::variables_map &vm) {
  ...
  // set algorithm for Ceres minimiser
  ...
  if (defaultMinimizerType_ == "Ceres") {
    ...
    if (!allowed.count(algo)) {
      CombineLogger::instance().log("CascadeMinimizer.cc",
                                    __LINE__,
                                    Form("Unknown Ceres algorithm %s, defaulting to TrustRegion", algo.c_str()),
                                    __func__);
      algo = "TrustRegion";
    }
  ...
  if (vm.count("cminFallbackAlgo")) {
    ...
    for (vector<string>::const_iterator it = falls.begin(), ed = falls.end(); it != ed; ++it) {
      ...
      if (idx != string::npos && idx < algo.length()) {
        ...
        if ('0' <= algo[idx + 1] && algo[idx + 1] <= '9') {
          ...
          if (ft != minimizerAlgoMap_.end()) {
            ...
            algo = std::string(defaultMinimizerAlgo_);

        } else {
          // otherwise, it could be Name,subname,strategy
          std::vector<std::string> configs = Utils::split(algo, ",");
          if (configs.size() != 3) {
            std::cerr << "The fallback command from --cminFallbackAlgo " << *it
                      << " is malformed. It should be formatted as Type[,Algo],strategy[:tolerance] " << std::endl;
            exit(0);
          }
          type = configs[0];
          algo = configs[1];
      ...
      if (!checkAlgoInType(type, algo)) {
        std::cerr << Form(
            "The fallback combination of minimizer type/algo %s/%s, is not recognized. Please check --cminFallbackAlgo "
            "again",
            type.c_str(),
            algo.c_str());
        exit(0);
      ...
      CombineLogger::instance().log(
          "CascadeMinimizer.cc",
          __LINE__,
          std::string(Form("Configured fallback algorithm, type %s, algo %s, strategy %d, tolerance %g",
                           fallbacks_.back().type.data(),
                           fallbacks_.back().algo.data(),
                           fallbacks_.back().strategy,
                           fallbacks_.back().tolerance)),
          __func__);
    }
  }

  if (defaultMinimizerType_ == "Minuit2") {
  ...
  if (vm.count("cminCeresMaxIterations")) {
    ...
    if (val < 1) {
      val = 1;
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresMaxIterations must be >0, using 1", __func__);
    }
  ...
  if (vm.count("cminCeresLinearSolver")) {
    ...
    static const std::set<std::string> allowed{
        "dense_qr", "dense_normal_cholesky", "iterative_schur", "sparse_normal_cholesky", "dense_schur", "sparse_schur"};
    if (!allowed.count(v)) {
      CombineLogger::instance().log("CascadeMinimizer.cc",
                                    __LINE__,
                                    Form("Unknown Ceres linear solver %s, defaulting to dense_qr", v.c_str()),
                                    __func__);
      v = "dense_qr";
    }
  ...
  if (vm.count("cminCeresMultiStart")) {
    ...
    if (val < 1) {
      val = 1;
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresMultiStart must be >0, using 1", __func__);
    }
  ...
  if (vm.count("cminCeresJitter")) {
    ...
    if (val < 0) {
      val = 0.0;
      CombineLogger::instance().log("CascadeMinimizer.cc", __LINE__, "cminCeresJitter must be >=0, using 0", __func__);
    }
  ...
  if (vm.count("cminCeresNumThreads")) {
    ...
    if (val < 1) {
      val = 1;
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresNumThreads must be >0, using 1", __func__);
    }
  ...
  if (vm.count("cminCeresLogFile")) {
    ...
    if (!f.empty())
      setenv("CERES_LOG_FILE", f.c_str(), 1);
  }
  if (vm.count("cminCeresFunctionTolerance")) {
    double val = vm["cminCeresFunctionTolerance"].as<double>();
    if (val <= 0) {
      val = defaultMinimizerTolerance_;
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresFunctionTolerance must be >0, using default", __func__);
    }
  ...
  if (vm.count("cminCeresGradientTolerance")) {
    ...
    if (val <= 0) {
      val = defaultMinimizerTolerance_;
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresGradientTolerance must be >0, using default", __func__);
    }
  ...
  if (vm.count("cminCeresParameterTolerance")) {
    ...
    if (val <= 0) {
      val = defaultMinimizerTolerance_;
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresParameterTolerance must be >0, using default", __func__);
    }
  ...
  if (vm.count("cminCeresNumericDiffStep")) {
    ...
    if (val <= 0) {
      val = 1e-4;
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresNumericDiffStep must be >0, using 1e-4", __func__);
    }
  ...
  if (vm.count("cminCeresDiffMethod")) {
    ...
    if (!allowed.count(v)) {
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, Form("Unknown diff method %s, defaulting to central", v.c_str()), __func__);
      v = "central";
    }
  ...
  if (vm.count("cminCeresLossFunction")) {
    ...
    if (!allowed.count(v)) {
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, Form("Unknown loss function %s, defaulting to none", v.c_str()), __func__);
      v = "none";
    }
  ...
  if (vm.count("cminCeresLossScale")) {
    ...
    if (val <= 0) {
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresLossScale must be >0, using 1", __func__);
      val = 1.0;
    }
  ...
  if (vm.count("cminCeresInitialRadius")) {
    ...
    if (val <= 0) {
      val = 1.0;
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresInitialRadius must be >0, using 1", __func__);
    }
  ...
  if (vm.count("cminCeresMaxTime")) {
    ...
    if (val < 0) {
      val = 0;
      CombineLogger::instance().log("CascadeMinimizer.cc", __LINE__, "cminCeresMaxTime must be >=0, using 0", __func__);
    }
    if (val > 0)
      setenv("CERES_MAX_TIME", std::to_string(val).c_str(), 1);
  }
  if (vm.count("cminCeresJitterDist")) {
    ...
    if (!allowed.count(v)) {
      CombineLogger::instance().log("CascadeMinimizer.cc",
                                    __LINE__,
                                    Form("Unknown jitter distribution %s, defaulting to uniform", v.c_str()),
                                    __func__);
      v = "uniform";
    }
  ...
  if (vm.count("cminCeresBoundRelax")) {
    ...
    if (val < 0) {
      val = 0;
      CombineLogger::instance().log(
          "CascadeMinimizer.cc", __LINE__, "cminCeresBoundRelax must be >=0, using 0", __func__);
    }
    if (val > 0)
      setenv("CERES_BOUND_RELAX", std::to_string(val).c_str(), 1);
  }
...
void CascadeMinimizer::printCeresConfig(int verbose) {
  if (verbose <= 0 || defaultMinimizerType_ != "Ceres") return;
...
bool CascadeMinimizer::autoBoundsOk(int verbose) {
    ...
    for (RooAbsArg *a : *pois) {
      ...
      if (rrv && !rrv->isConstant() && rrv->hasMax() && rrv->hasMin()) {
        ...
        if (bothBounds && val < (0.9 * lo + 0.1 * hi)) {
          ...
          if (verbose)
            std::cout << " POI " << rrv->GetName() << " is at " << val << ", within 10% from the lower boundary " << lo
                      << ". Will enlarge range to [ " << rrv->getMin() << " , " << hi << " ]" << std::endl;
        } else if (val > (0.9 * hi + 0.1 * lo)) {
          ok = false;
          rrv->setMax(val + (val - lo));
          if (verbose)
            std::cout << " POI " << rrv->GetName() << " is at " << val << ", within 10% from the upper boundary " << hi
  ...
  if (!ok && verbose) {
    ...
    CombineLogger::instance().log("CascadeMinimizer.cc",
                                  __LINE__,
                                  "On checking with autoBounds on, at least one of the POIs was close to the boundary, "
                                  "combine is now repeating the fit.",
                                  __func__);
  }
  return ok;
}
