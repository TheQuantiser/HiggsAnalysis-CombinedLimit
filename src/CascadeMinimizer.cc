...
void CascadeMinimizer::applyOptions(const boost::program_options::variables_map &vm) 
{
    ...
    // ensure sensible defaults when selecting Ceres without an explicit algorithm
    if (defaultMinimizerType_ == "Ceres" && vm["cminDefaultMinimizerAlgo"].defaulted()) {
        defaultMinimizerAlgo_ = "TrustRegion";
    }

    // allow --cminCeresAlgo to override the algorithm and validate it
    if (vm.count("cminCeresAlgo")) {
        std::string algo = vm["cminCeresAlgo"].as<std::string>();
        if (algo == "0") algo = "TrustRegion";
        else if (algo == "1") algo = "LineSearch";
        ...
        if (defaultMinimizerType_ != "Ceres" && vm["cminDefaultMinimizerType"].defaulted() && !vm.count("cminUseCeres")) {
            defaultMinimizerType_ = "Ceres";
        }
    }

    // check default minimizer type/algo if the user explicitly set either option
    if (!vm["cminDefaultMinimizerType"].defaulted() || !vm["cminDefaultMinimizerAlgo"].defaulted()) {
    ...
    if (vm["cminUseCeres"].as<bool>()) {
        defaultMinimizerType_ = "Ceres";
        if (vm["cminDefaultMinimizerAlgo"].defaulted() && !vm.count("cminCeresAlgo")) {
            defaultMinimizerAlgo_ = "TrustRegion";
        }
    }
    ...
    CombineLogger::instance().log("CascadeMinimizer.cc",__LINE__,
        Form("Using minimizer %s/%s with tolerance %.6g", defaultMinimizerType_.c_str(), defaultMinimizerAlgo_.c_str(), defaultMinimizerTolerance_),
        __func__);
