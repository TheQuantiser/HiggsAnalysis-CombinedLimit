  ...
  options_.add_options()
      ...
      ("preFitValue",
       ...
       "(note this overrides --expectSignal for these features)")(
          ...
          "Also compute 2-sigma interval from delta(nll) = 1.92 instead of 0.5")(
          "robustFit",
          ...
          "Adjust bounds for these POIs if they end up close to the boundary. Can be a list of POIs, or \"*\" to get "
          "all")("autoMaxPOIs",
                 boost::program_options::value<std::string>(&autoMaxPOIs_)->default_value(autoMaxPOIs_),
                 "Adjust maxima for these POIs if they end up close to the boundary. Can be a list of POIs, or \"*\" "
                 "to get all")(
          "forceRecreateNLL",
          "Always recreate NLL when running on multiple toys rather than re-using nll with new dataset")(
          "nllbackend",
          boost::program_options::value<std::string>(&Combine::nllBackend())->default_value(Combine::nllBackend()),
          "DEBUG OPTION, DO NOT USE! Set backend to create NLL. Choices: combine (default behavior), cpu, legacy, "
          "codegen")
  ...
  if (robustFit_) {
    if (verbose) {
      CombineLogger::instance().log(
          "FitterAlgoBase.cc",
          __LINE__,
          std::string(Form("Setting robust fit options to Tolerance=%g / Strategy=%d / Type,Algo=%s (note that "
                           "defaults of CascadeMinimizer if these options were not specified)",
                           minimizerToleranceForMinos_,
                           minimizerStrategyForMinos_,
                           minimizerAlgoForMinos_.c_str())),
          __func__);
    }
...
bool FitterAlgoBase::run(RooWorkspace *w,
  ...
  if (shouldCreateNLLBranch) {
    Combine::addBranch("nll", &nllValue_, "nll/D");
    Combine::addBranch("nll0", &nll0Value_, "nll0/D");
    shouldCreateNLLBranch = false;
  }
...
RooFitResult *FitterAlgoBase::doFit(RooAbsPdf &pdf,
  ...
  if (runtimedef::get("SETPARAMETERS_AFTER_NLL")) {
    ...
    if (verbose >= 3)
      CombineLogger::instance().log("FitterAlgoBase.cc",
                                    __LINE__,
                                    std::string(Form("DELTA NLL FROM SETPARAMETERS = %f", nll->getVal() - nll0)),
                                    __func__);
  }
  ...
  CascadeMinimizer minim(
      *nll, CascadeMinimizer::Unconstrained, rs.getSize() ? dynamic_cast<RooRealVar *>(rs.first()) : 0);
  ...
  if (verbose > 1)
    CombineLogger::instance().log("FitterAlgoBase.cc", __LINE__, "do first minimization", __func__);
  TStopwatch tw;
  if (verbose)
    tw.Start();
  ...
  if (verbose > 1)
    CombineLogger::instance().log(
        "FitterAlgoBase.cc",
        __LINE__,
        std::string(Form("Minimized in %f seconds (%f CPU time)", tw.RealTime(), tw.CpuTime())),
        __func__);
  nll0Value_ = nll0;
  nllValue_ = nll->getVal() - nll0;
  if (verbose >= 3) {
    CombineLogger::instance().log(
        "FitterAlgoBase.cc", __LINE__, std::string(Form("FINAL NLL - NLL0 VALUE = %.10g\n", nllValue_)), __func__);
    if (CascadeMinimizerGlobalConfigs::O().pdfCategories.getSize() > 0) {
      ...
      for (unsigned int ic = 0, nc = CascadeMinimizerGlobalConfigs::O().pdfCategories.getSize(); ic != nc; ++ic) {
        ...
        CombineLogger::instance().log(
            "FitterAlgoBase.cc",
            __LINE__,
            std::string(Form("%s%s=%d", (ic > 0 ? "," : ""), cat->GetName(), cat->getIndex())),
            __func__);
      }
    }
  }
  ...
  for (int i = 0, n = rs.getSize(); i < n; ++i) {
    ...
    if (!rfloat && runtimedef::get("MINIMIZER_no_analytic")) {
      fprintf(sentry.trueStdOut(),
              "Skipping %s. Looks like the last fit did not float this parameter. You could try running --algo grid to "
    ...
    if (!robustFit_) {
      ...
      if (verbose) {
        CombineLogger::instance().log(
            "FitterAlgoBase.cc", __LINE__, std::string(Form("Running Minos for POI %s", r.GetName())), __func__);
      }
      ...
      if (verbose) {
        ...
        CombineLogger::instance().log("FitterAlgoBase.cc",
                                      __LINE__,
                                      std::string(Form("Running RobustFit for POI %s. Configured with strategy %d  ",
                                                       r.GetName(),
                                                       minimizerStrategyForMinos_)),
                                      __func__);
      }
      ...
      double hi95 = do95_ ? findCrossing(minim2,
                                         *nll,
                                         r,
                                         threshold95,
                                         std::isnan(hi68) ? r0 : hi68,
                                         std::max(rMax, std::isnan(hi68 * 2 - r0) ? r0 : hi68 * 2 - r0))
                          : r0;
      // low error
      *allpars = RooArgSet(ret->floatParsFinal());
      r.setVal(r0);
      r.setConstant(true);
...
double FitterAlgoBase::findCrossing(
  ...
  if (verbose) {
    ...
    CombineLogger::instance().log(
        ...
        __LINE__,
        std::string(Form(
            "Searching for crossing at nll = %g, in the interval %g < %s < %g", level, rStart, r.GetName(), rBound)),
        __func__);
  }
  ...
  if (!ok && !keepFailures_) {
    ...
    CombineLogger::instance().log("FitterAlgoBase.cc",
                                  __LINE__,
                                  std::string(Form("[ERROR] Minimization failed at %s = %g", r.GetName(), rStart)),
                                  __func__);
    return NAN;
  }
    ...
    if (!ok && !keepFailures_) {
      ...
      if (nfail >= maxFailedSteps_) {
        ...
        CombineLogger::instance().log(
            "FitterAlgoBase.cc",
            __LINE__,
            std::string(Form("Maximum failed steps (max=%d) reached and minimization failed at %s = %g ",
                             maxFailedSteps_,
                             r.GetName(),
                             rStart)),
            __func__);
      ...
      if (allpars.get() == 0)
        allpars.reset(nll.getParameters((const RooArgSet *)0));
      *allpars = oldparams;
      rStart -= rInc;
      rInc *= 0.5;
      continue;
  ...
  if (fabs(here - level) > 0.01) {
    ...
    CombineLogger::instance().log(
        "FitterAlgoBase.cc", __LINE__, "[ERROR] Closed range without finding crossing! ", __func__);
    return NAN;
  } else {
    return r.getVal();
  }
...
double FitterAlgoBase::findCrossingNew(
  ...
  if (verbose) {
    ...
    CombineLogger::instance().log(
        "FitterAlgoBase.cc",
        __LINE__,
        std::string(Form("Searching for crossing at nll = %g in the interval [ %g , %g ] ", level, rStart, rBound)),
        __func__);
  }
  ...
  for (int iter = 0; iter < 20; ++iter) {
    ...
    if (nll.numEvalErrors() > 0 || std::isnan(yStart) || std::isinf(yStart)) {
      ...
      CombineLogger::instance().log(
          ...
          __LINE__,
          std::string(
              Form("[ERROR] logEvalErrors reported from NLL on start of loop for iteration %d, x %+10.6f", iter, rVal)),
          __func__);
    }
    ...
    if (verbose > 1) {
      ...
      CombineLogger::instance().log(
          "FitterAlgoBase.cc",
          __LINE__,
          std::string(Form(" x %+10.6f   y %+10.6f    step %+10.6f [ START OF ITER %d, bound %+10.6f ]",
                           rVal,
                           yStart - level,
                           rInc,
                           iter,
    ...
    while (unbound || (rBound - rVal - rInc) * rInc >= 0) {  // if I've not yet reached the boundary
      ...
      if (r.getVal() != rVal) {
        ...
        CombineLogger::instance().log("FitterAlgoBase.cc",
                                      __LINE__,
                                      std::string(Form("[ERROR] can't set %s = %g\n", r.GetName(), rVal)),
                                      __func__);
      }
      nll.clearEvalErrorLog();
      double y = nll.getVal();
      if (nll.numEvalErrors() > 0 || std::isnan(y) || std::isinf(y) || fabs(y - level) > 1e6) {
        ...
        if (verbose > 1)
          CombineLogger::instance().log(
              ...
              __LINE__,
              std::string(Form(
                  "logEvalErrors reported from NLL on stepping for iteration %d, set range to  [ %+10.6f, %+10.6f ]",
                  iter,
                  rStart,
                  rVal)),
              __func__);
      ...
      if (verbose > 1) {
        ...
        CombineLogger::instance().log(
            "FitterAlgoBase.cc",
            __LINE__,
            std::string(Form(" x %+10.6f   y %+10.6f   yCorr %+10.6f", rVal, y - level, yCorr - level)),
            __func__);
      }
      if (fabs(yCorr - yStart) > 0.7) {
        ...
        if (verbose > 1) {
          ...
          CombineLogger::instance().log(
              "FitterAlgoBase.cc", __LINE__, " --------> accumulated big change in NLL, will minimize", __func__);
        }
        break;
      }
      if ((level - yCorr) * (level - yStart) < 0) {
        ...
        for (int iter2 = 0; (fabs(yCorr - level) > minimizerTolerance_) && iter2 < 5; ++iter2) {
          ...
          if (verbose > 1) {
            ...
            CombineLogger::instance().log(
                "FitterAlgoBase.cc",
                __LINE__,
                std::string(Form("x %+10.6f   y %+10.6f   yCorr %+10.6f   [ bisection iter %d in  %+10.6f in %+10.6f ]",
                                 rMid,
                                 y - level,
                                 yCorr - level,
                                 iter2,
        ...
        if (verbose > 1) {
          ...
          CombineLogger::instance().log(
              "FitterAlgoBase.cc", __LINE__, std::string(Form(" --------> ending with x %+10.6f", rVal)), __func__);
        }
        hitbound = false;
        break;
      }
    ...
    if (!minim.improve(verbose - 1)) {
      ...
      CombineLogger::instance().log("FitterAlgoBase.cc",
                                    __LINE__,
                                    std::string(Form("[ERROR] Minimization failed at %s = %g", r.GetName(), rVal)),
                                    __func__);
      if (!neverGiveUp)
        return NAN;
    }
    ...
    if (verbose > 1) {
      ...
      CombineLogger::instance().log("FitterAlgoBase.cc",
                                    __LINE__,
                                    std::string(Form(" x %+10.6f   y %+10.6f   yCorr %+10.6f   yProf  %+10.6f   (P-U) "
                                                     "%+10.6f    (P-C) %+10.6f    oldSlope %+10.6f    newSlope %+10.6f",
                                                     rVal,
                                                     yUnprof - level,
                                                     yCorr - level,
                                                     yProf - level,
                                                     ...
                                                     yProf - yCorr,
                                                     quadCorr,
                                                     (yUnprof - yProf) / std::pow(rVal - rStart, 2))),
                                    __func__);
    }
    ...
    if ((level - yStart) * (level - yProf) > 0) {
      ...
      if (hitbound) {
        ...
        CombineLogger::instance().log(
            "FitterAlgoBase.cc", __LINE__, "Closed range without finding crossing! ", __func__);
        return rVal;
      } else {
        if (verbose > 1) {
          ...
          CombineLogger::instance().log("FitterAlgoBase.cc",
                                        __LINE__,
                                        std::string(Form(" ---> change search window to [ %g , %g ]", rStart, rBound)),
                                        __func__);
        }
      }
    } else {
  ...
  CombineLogger::instance().log(
      "FitterAlgoBase.cc",
      __LINE__,
      std::string(Form("[WARNING] Search for crossing did not converge, will return approximate answer %g", rVal)),
      __func__);
  return rVal;
}
