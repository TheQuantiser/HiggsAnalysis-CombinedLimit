  ...
  options_.add_options()("rAbsAcc",
      ...
      ("qtilde",
       ...
       "Allow only non-negative signal strengths (default is true).")(
          ...
          "picky", "Abort on fit failures")("noFitAsimov", "Use the pre-fit asimov dataset")(
          "getLimitFromGrid",
...
void AsymptoticLimits::applyOptions(const boost::program_options::variables_map &vm) {
  if (vm.count("singlePoint") && !vm["singlePoint"].defaulted()) {
    ...
    if (what_ != "observed" && what_ != "expected" && what_ != "both" && what_ != "blind")
      throw std::invalid_argument(
          "AsymptoticLimits: option 'run' can only be 'observed', 'expected', 'both' (the default) or 'blind' "
          "(a-priori expected)");
  }
  ...
  if (rule_ == "CLs")
    ...
    throw std::invalid_argument("AsymptoticLimits: Rule must be either 'CLs' or 'Pmu'");
  ...
  if (noFitAsimov_)
    std::cout << "Will use a-priori instead of a-posteriori expected background." << std::endl;
...
void AsymptoticLimits::applyDefaultOptions() {
  what_ = "observed";
  noFitAsimov_ = true;  // faster
}

bool AsymptoticLimits::run(RooWorkspace *w,
  ...
  for (RooAbsArg *a : *params_) {
    if (a->IsA()->InheritsFrom(RooCategory::Class())) {
      hasDiscreteParams_ = true;
      break;
    }
  }
  ...
  if (what_ == "both" || what_ == "expected")
    expected = runLimitExpected(w, mc_s, mc_b, data, limit, limitErr, hint);
  if (what_ != "expected")
  ...
  if (verbose >= 0) {
    ...
    for (std::vector<std::pair<float, float> >::const_iterator it = expected.begin(), ed = expected.end(); it != ed;
         ++it) {
...
bool AsymptoticLimits::runLimit(RooWorkspace *w,
                                RooStats::ModelConfig *mc_s,
                                RooStats::ModelConfig *mc_b,
                                RooAbsData &data,
                                double &limit,
  ...
  for (RooAbsArg *a : *params_) {
    ...
    if (rrv != 0 && rrv != r && rrv->isConstant() == false) {
      hasFloatParams_ = true;
      break;
    }
  }
  ...
  if (verbose > 0) {
    ...
    CombineLogger::instance().log(
        "AsymptoticLimits.cc",
        __LINE__,
        std::string(Form("NLL at global minimum of data = %g (%s=%g)", minNllD_, r->GetName(), r->getVal())),
        __func__);
  }
  ...
  if (verbose > 0) {
    ...
    CombineLogger::instance().log(
        "AsymptoticLimits.cc",
        __LINE__,
        std::string(Form("NLL at global minimum of asimov = %g (%s=%g)", minNllA_, r->GetName(), r->getVal())),
        __func__);
  }
  if (verbose > 2)
    fitFreeA_.Print("V");
  ...
  for (int tries = 0; tries < 5; ++tries) {
    ...
    if (cls == -999) {
      ...
      CombineLogger::instance().log("AsymptoticLimits.cc",
                                    __LINE__,
                                    "[ERROR] Minimization failed in an unrecoverable way for calculation of limit",
                                    __func__);
      break;
    }
    if (cls < clsTarget) {
    ...
    if (strictBounds_ && rMax == r->getMax()) {
      ...
      CombineLogger::instance().log(
          "AsymptoticLimits.cc",
          __LINE__,
          std::string(Form(" %s at upper bound %s = %g is %g. Stopping search and using upper bound as limit.",
                           rule_.data(),
                           r->GetName(),
                           r->getVal(),
                           cls)),
  ...
  do {
    ...
    if (cls == -999) {
      ...
      CombineLogger::instance().log("AsymptoticLimits.cc",
                                    __LINE__,
                                    "Minimization failed in an unrecoverable way for calculation of limit",
                                    __func__);
      break;
    }
    if (cls > clsTarget) {
...
double AsymptoticLimits::getCLs(RooRealVar &r, double rVal, bool getAlsoExpected, double *limit, double *limitErr) {
  if (strictBounds_ && rVal > r.getMax())
    throw std::runtime_error("Overflow in getCLs");
  if (!strictBounds_)
    r.setMax(1.1 * rVal);
  r.setConstant(true);
  ...
  if (what_ == "singlePoint" && rVal < rBestD_) {
    if (verbose > 0)
      CombineLogger::instance().log(
          "AsymptoticLimits.cc",
          __LINE__,
          std::string(Form("Value being tested (%s=%f) is lower than the best fit(%s=%f). Setting q_mu to zero.",
                           r.GetName(),
                           rValue_,
                           r.GetName(),
                           rBestD_)),
  ...
  *params_ = snapGlobalObsAsimov;
  ...
  if (verbose > 0) {
    CombineLogger::instance().log(
        "AsymptoticLimits.cc",
        __LINE__,
        std::string(Form("At %s = %f:\tq_mu = %.5f\tq_A  = %.5f\tPmu = %7.5f\t1-Pb  = %7.5f\tCLs  = %7.5f",
                         r.GetName(),
                         rVal,
                         qmu,
                         qA,
  ...
  if (getAlsoExpected) {
    ...
    for (int iq = 0; iq < 5; ++iq) {
      ...
      if (verbose > 0)
        printf("Expected %4.1f%%: Pmu = %.5f  1-Pb = %.5f   CLs = %.5f\n", quantiles[iq] * 100, pmu, pb, pmu / pb);
    }
  }
  return doCLs_ ? CLs : Pmu;
}

std::vector<std::pair<float, float> > AsymptoticLimits::runLimitExpected(RooWorkspace *w,
                                                                         RooStats::ModelConfig *mc_s,
                                                                         RooStats::ModelConfig *mc_b,
                                                                         RooAbsData &data,
                                                                         double &limit,
  ...
  *params_ = snapGlobalObsAsimov;
  ...
  if (r->getVal() / r->getMax() > 1e-3) {
    if (verbose) {
      CombineLogger::instance().log(
          ...
          __LINE__,
          std::string(
              Form("[WARNING] Best fit of asimov dataset is at %s = %f (%f times %sMax), while it should be at zero",
                   r->GetName(),
                   r->getVal(),
                   r->getVal() / r->getMax(),
                   r->GetName())),
  ...
  if (verbose > 0) {
    ...
    CombineLogger::instance().log(
        "AsymptoticLimits.cc",
        __LINE__,
        std::string(Form("Median for expected limits = %g, Sigma for expected limits = %g", median, sigma)),
        __func__);
  }
  ...
  for (int iq = 0; iq < 5; ++iq) {
    ...
    if (newExpected_ && iq != 2) {  // the median is exactly the same in the two methods
      ...
      if (minosAlgo_ == "stepping")
        minosAlgo_ = "bisection";
      switch (iq) {
        case 0:
          limit = findExpectedLimitFromCrossing(*nll, r, r->getMin(), median, nll0, quantiles[iq]);
          break;
        case 1:
          limit = findExpectedLimitFromCrossing(*nll, r, expected.back().second, median, nll0, quantiles[iq]);
          break;
        case 3:
          limit =
              findExpectedLimitFromCrossing(*nll, r, expected.back().second, median + 2 * sigma, nll0, quantiles[iq]);
          break;
        case 4:
          limit =
              findExpectedLimitFromCrossing(*nll, r, expected.back().second, median + 4 * sigma, nll0, quantiles[iq]);
          break;
      }
    ...
    if (strictBounds_ && limit > r->getMax())
      limit = r->getMax();
    limitErr = 0;
    Combine::commitPoint(true, quantiles[iq]);
    expected.push_back(std::pair<float, float>(quantiles[iq], limit));
...
float AsymptoticLimits::findExpectedLimitFromCrossing(
    RooAbsReal &nll, RooRealVar *r, double rMin, double rMax, double nll0, double pb) {
  ...
  if (minosAlgo_ == "minos") {
    ...
    for (int tries = 0; tries < 3; ++tries) {
      ...
      if (tries == 1) {
        ...
        if (minim.algo().find("Minuit2") != std::string::npos) {
          ...
          minim.minimizer().minimize(type.c_str(), algo.c_str());
        }
      }
    }
    if (minosStat != -1)
      return r->getVal() + r->getAsymErrorHi();
    minim.minimizer().hesse();
    CombineLogger::instance().log("AsymptoticLimits.cc", __LINE__, "Minos failed; using Hesse errors", __func__);
    ...
    if (minosAlgo_ == "bisection") {
      if (verbose > 1)
        CombineLogger::instance().log(
            "AsymptoticLimits.cc", __LINE__, "Will search for NLL crossing by bisection", __func__);
      if (strictBounds_)
      ...
      while (rErr > std::max(rRelAccuracy_ * rCross, rAbsAccuracy_)) {
        ...
        if (verbose > 1)
          CombineLogger::instance().log(
              "AsymptoticLimits.cc",
              __LINE__,
              std::string(Form("At %s = %f:\tdelta(nll) = %.5f\n", r->GetName(), rCross, here - nll0)),
              __func__);
        ...
        if (here < threshold)
          ...
          rMax = rCross;
        rCross = 0.5 * (rMin + rMax);
        rErr = 0.5 * (rMax - rMin);
      }
    } else if (minosAlgo_ == "stepping") {
      if (verbose > 1)
        CombineLogger::instance().log(
            "AsymptoticLimits.cc", __LINE__, "Will search for NLL crossing by stepping", __func__);
      rCross = 0.05 * rMax;
      rErr = rMax;
      double stride = rCross;
        ...
        if (here < threshold) {
          ...
          if (strictBounds_ && rCross == r->getMax()) {
            if (verbose > 1)
              CombineLogger::instance().log("AsymptoticLimits.cc",
                                            __LINE__,
                                            std::string(Form("reached hard bound at %s = %f\n", r->GetName(), rCross)),
                                            __func__);
            return rCross;
          }
          rCross += stride;
      ...
      if (verbose > 1)
        CombineLogger::instance().log(
            "AsymptoticLimits.cc", __LINE__, "Will search for NLL crossing with new algorithm", __func__);
      //
        ...
        if (binNLLchange || aboveThresh) {
          ...
          if (verbose > 1)
            CombineLogger::instance().log(
                "AsymptoticLimits.cc",
                __LINE__,
                std::string(Form("At %s = %f:\tdelta(nll unprof) = %.5f\tdelta(nll prof) = %.5f\tkappa=%.5f\n",
                                 r->GetName(),
                                 r_1,
                                 nll_1 - nll0,
                                 nll.getVal() - nll0,
          ...
          if (verbose > 1)
            CombineLogger::instance().log(
                "AsymptoticLimits.cc",
                __LINE__,
                std::string(Form("At %s = %f:\tdelta(nll unprof) = %.5f\t                         \tkappa=%.5f\n",
                                 r->GetName(),
                                 r_1,
                                 nll_1 - nll0,
                                 kappa)),
      ...
      if (verbose > 1)
        CombineLogger::instance().log(
            "AsymptoticLimits.cc",
            __LINE__,
            std::string(Form("At %s = %f:\t                         \tdelta(nll prof) = %.5f\tkappa=%.5f\n",
                             r->GetName(),
                             r_0,
                             nll_0 - nll0,
                             kappa)),
      ...
      do {
        ...
        while (r_hi - r_lo > rEps) {
          ...
          if (verbose > 1)
            CombineLogger::instance().log(
                "AsymptoticLimits.cc",
                __LINE__,
                std::string(Form("At %s = %f:\tdelta(nll unprof) = %.5f\tdelta(nll appr) = %.5f\tkappa=%.5f\n",
                                 r->GetName(),
                                 r_2,
                                 y0 - nll0,
                                 y - nll0,
        ...
        if (verbose > 1)
          CombineLogger::instance().log(
              ...
              __LINE__,
              std::string(
                  Form("At %s = %f:\tdelta(nll unprof) = %.5f\tdelta(nll prof) = %.5f\tdelta(nll appr) = %.5f\n",
                       r->GetName(),
                       rCross,
                       nll_unprof - nll0,
                       nll_prof - nll0,
                       nll_unprof - nll0 - kappa * std::pow(rCross - r_1, 2))),
              __func__);
        if (fabs(nll_prof - threshold) < 0.1 * minim2.tolerance()) {
          break;
        }
  ...
  if (verbose > 1)
    CombineLogger::instance().log(
        "AsymptoticLimits.cc",
        __LINE__,
        std::string(Form("[WARNING] search for crossing of %s between %f and %f failed", r->GetName(), rMin, rMax)),
        __func__);
  return std::numeric_limits<float>::quiet_NaN();
}
...
float AsymptoticLimits::calculateLimitFromGrid(RooRealVar *r, double quantile, double alpha) {
  ...
  for (; iq < 6; iq++) {
    if (fabs(quantile - quantiles[iq]) < 0.001)
      break;  // this is a pretty lame way to find the right entry
  }
  ...
  if (!rminfound) {
    ...
    CombineLogger::instance().log(
        "AsymptoticLimits.cc",
        __LINE__,
        std::string(Form("Cannot find r with CL above threshold for quantile %g, using lowest value of r found",
                         quantiles[iq])),
        __func__);
    return rlower;
  }
  if (!rmaxfound) {
    ...
    CombineLogger::instance().log(
        "AsymptoticLimits.cc",
        __LINE__,
        std::string(Form("Cannot find r with CL below threshold for quantile %g, using largest value of r found",
                         quantiles[iq])),
        __func__);
    return rupper;
  }
...
RooAbsData *AsymptoticLimits::asimovDataset(RooWorkspace *w,
                                            RooStats::ModelConfig *mc_s,
                                            RooStats::ModelConfig *mc_b,
                                            RooAbsData &data) {
  // Do this only once
